...
public class BloodiedDiverLayer extends RenderLayer<InfectedDiverEntity, HumanoidModel<InfectedDiverEntity>> {
    protected final static ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Constants.MOD_ID, "textures/entity/base.png");

    public BloodiedDiverLayer(RenderLayerParent<InfectedDiverEntity, HumanoidModel<InfectedDiverEntity>> entityRendererIn) {
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, InfectedDiverEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if(!entitylivingbaseIn.isInvisible()){
            ...
            this.getParentModel().renderToBuffer(matrixStackIn, bb, packedLightIn, OverlayTexture.NO_OVERLAY);
        }
    }
}
